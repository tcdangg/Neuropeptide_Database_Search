# -*- coding: utf-8 -*-
"""
Created on Wed Dec  7 15:05:03 2022
...
output_folder = r"C:\Users\lawashburn\Documents\DBpep_v2\results_log\20221201\v17" #folder in which all output directories will be generated
...
modded_target_decoy_DB = pd.DataFrame()
modded_target_decoy_DB['Modified Sequence'] = final_seq_list

file_out_path = output_folder + '\\modified_target_decoy_DB.csv'
with open(file_out_path,'w',newline='') as filec:
        writerc = csv.writer(filec)
        modded_target_decoy_DB.to_csv(filec,index=False)
...
# =============================================================================
        ...
        for y in scans_present:
            ...
            for l in scan_filtered_pep:
                ...
                #frag_favor_step_index = stepped_index[stepped_index['Fragment actual intensity'] != 0]
                #frag_favor_step_index = frag_favor_step_index[frag_favor_step_index['Theoretical intensity'] != 0]
                #frag_favor_step_index = frag_favor_step_index.drop_duplicates()
                # frag_favor_step_index = stepped_index.sort_values(by='Fragment actual intensity',ascending=False)
                #frag_favor_step_index['Fragment ion'] = frag_favor_step_index['Fragment ion'].str.replace('-H2O','')
                #frag_favor_step_index['Fragment ion'] = frag_favor_step_index['Fragment ion'].str.replace('-NH3','')
                #frag_favor_step_index = frag_favor_step_index.sort_values(by='Fragment actual intensity',ascending=False)
                # frag_favor_step_index = frag_favor_step_index.drop_duplicates(subset='Fragment ion')
                ...
                normalization_multiple_factor = 50/normalization_factor
                filter_xcorr_input['Fragment normalized intensity'] = filter_xcorr_input['Fragment actual intensity']*normalization_multiple_factor
                filter_xcorr_input = filter_xcorr_input.drop_duplicates()
                file_path = peptide_report_output+'\\'+peptide+'_'+str(scan_to_report)+'_step_fragment_matchesv2.csv'
                with open(file_path,'w',newline='') as filec:
                        writerc = csv.writer(filec)
...
file_path = output_folder + '\\final_report_wo_corr.csv'
with open(file_path,'w',newline='') as filec:
        writerc = csv.writer(filec)
...
file_path = output_folder + '\\corr_report.csv'
with open(file_path,'w',newline='') as filec:
        writerc = csv.writer(filec)
        corr_report.to_csv(filec,index=False) 
