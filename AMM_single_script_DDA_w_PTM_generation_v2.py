...
output_folder = r"C:\Users\lawashburn\Documents\DBpep_v2\results_log\20221201\v13" #folder in which all output directories will be generated
...
for peptide in candidate_sequences:
        ...
        for y in scans_present:
            ...
            for l in scan_filtered_pep:
                ###pull theoretical masses###
                plain_peptide = re.sub("[\(\[].*?[\)\]]", "",l)
                res_list_for_fragment = list_of_residues(l)
                mass_of_residues = []
                for residue in plain_peptide:
                    residue_mass = aa_masses[residue]
                    mass_of_residues.append(residue_mass)
                peptide_mass = (sum(mass_of_residues)) + check_termini(l) + check_PTM(l)
                mass_to_charge = (peptide_mass + (proton_mass * charge))/charge
                ...
                normalization_factor = stepped_index['Fragment actual intensity'].sum()
                normalization_multiple_factor = normalization/normalization_factor
                stepped_index['Fragment normalized intensity'] = stepped_index['Fragment actual intensity']*normalization_multiple_factor
                correlation = stepped_index['Theoretical intensity'].corr(stepped_index['Fragment normalized intensity'],method='pearson')
