# -*- coding: utf-8 -*-
"""
Created on Tue Mar  7 12:49:07 2023
...
working_directory = r'C:/Users/15156/Desktop/background 0'
...
path = r"C:\Users\15156\Desktop\background 0\best_intensity_0.csv"
...
for x in range(0,path_length):
  ...
  #  percent_spectrum_exp = percent_real[0]
   ...
   # pseudo_intensity_list = pseudo_intensity_list_2.split(",")
    ...
    pseudo_intensity_list = []
    pseudo_intensity_list.append(pseudo_intensity_list_2)
    for pseudo_intensity in pseudo_intensity_list:
        ...
        ion_nodup2 = list(set(ion_list)) #got 16 entries yay
        ion_nodup1 = []
        
        for a in ion_nodup2:
            ion_nodup1.append(str(a))
        
        ion_nodup = [k for k in ion_nodup1 if k != 'nan']
        ...
        for xx in range(1,rounds+1):
                ...
                for jj in range(0,xx):
                    ...
                    pseudo_spec1 = pseudo_spec1.dropna()
        ...
        filtered_results_df = results_df[results_df['Run #'] == rounds]
        average = (filtered_results_df['Average Score Pseudo Supplement'].values.tolist())[0]
        avg.append(average)
        background.append(exp_min_corr_score)
        fold_change = average / exp_min_corr_score 
        peptide_ID_storage.append(peptide_ID)
        fold_change_results.append(fold_change)
        exp_dp.append(exp_only_corr_score)
        ...
        plt.axhline(y=exp_min_corr_score,linewidth=2,color='#B1D877',label=('Background exp intensity '+str(exp_min_corr_score)))
...
fold_change_df = pd.DataFrame({'Backgrd DP' : background, 'Optimal avg DP' : avg, 'Experimental DP' : exp_dp})
fold_change_df.to_csv('0_bckgrd_1.csv', index=False)
