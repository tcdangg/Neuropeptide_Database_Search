...
output_parent_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\Troubleshoot\v5vsv6\v5" #folder in which all output directories will be generated
...
for raw_converter_path in raw_converter_path_input:
   ...
   # raw_converter = raw_converter['charge'].replace([0, 1], '', inplace=True)
    raw_converter['charge'] = raw_converter['charge'].replace(to_replace=0,value=1)
...
#%%    
 ...
 #%%   
    ...
    print('end of AMM', time.time())
    ##End of AMM, start XCorr by pulling theoretical spectra for all spectrum with a precursor match, all peaks from actual scan, calculate XCorr##
    #%%
    corr_correlation = []
    corr_pep = []
    corr_scan = []
    ...
    ctrl_og_array_store = []
    ctrl_modded_array_store = []
    ctrl_correlation_array_store = []
    correlation_ctrl_max_store = []
    correlation_control_mean_store = []
    ctrl_correlation_subtract_store = []
    ctrl_correlation_log_store = []
    
    for z in final_report_storage_peptide:
        filtered_final_report = final_report[final_report['Neuropeptide'] == z]
        applicable_scans = filtered_final_report['Scan'].values.tolist()
        
        ###pull theoretical spectra###
        plain_peptide = re.sub("[\(\[].*?[\)\]]", "",z) #removes any modification for mass calculations
        res_list_for_fragment = list_of_residues(z)
        mass_of_residues = []
        for residue in plain_peptide:
            residue_mass = aa_masses[residue]
            mass_of_residues.append(residue_mass)
        peptide_mass = (sum(mass_of_residues)) + check_termini(z) + check_PTM(z) #calculated MH mass
        mass_to_charge = (peptide_mass + (proton_mass * charge))/charge #translates the MH mass to m/z for each charge of interest
        ...
        for scan_search in applicable_scans:
            actual_peak_intensities = raw_converter[raw_converter['scan_number'] == scan_search]
            ...
            if normalization_factor == 0:
                normalization_multiple_factor = normalization
            else:
            ...
            filter_xcorr_input = filter_xcorr_input.drop_duplicates()
            filter_xcorr_input = filter_xcorr_input.drop_duplicates(subset=['Fragment ion', 'Fragment theoretical Monoisotopic Mass',
            ...
            filter_xcorr_input = filter_xcorr_input.round({'Fragment theoretical Monoisotopic Mass':4,'Fragment theoretical m/z':4,'Fragment normalized intensity':4})
            ...
            theoretical_sample_title = str(z) + '_' + str(scan_search) + '_theoretical_experimental_spectra.csv'
            ...
            correlation_array = signal.correlate(theoretical_array,actual_array,method='direct',mode='same')
            ...
            correlation_log =  np.log10(correlation)
    
            corr_correlation.append(correlation)
            corr_pep.append(z)
            corr_scan.append(scan_search)
            ...
            ##determine maximum possible xcorr##
            
            ctrl_og_array_store.append(theoretical_array)
    
            new_array = []
            
            for jj in theoretical_array:
                new_array.append(jj)
            
            number_theo_entries = new_array.count(normalization)
            number_of_charges_included = len(fragment_charges)
            
            number_entries_tokeep_1 = number_theo_entries*(1/number_of_charges_included) #accounts for multiple peaks from charges
            number_entries_tokeep_2 = number_entries_tokeep_1*(1/3) #accounts for adduct peaks
            number_entries_tokeep_2_round = int(number_entries_tokeep_2)
    
            indicies_in_whole_list = [i for i,val in enumerate(new_array) if val==normalization]
    
            indicies_to_keep = random.sample(indicies_in_whole_list, number_entries_tokeep_2_round)
    
            theo_to_remove = []
            for x in indicies_in_whole_list:
                if x not in indicies_to_keep:
                    theo_to_remove.append(x)
    
            
    
            replacements = []
            
            for xx in theo_to_remove:
                replacements.append(0)
    
            for ab in range(0,len(theo_to_remove)):
                new_array[theo_to_remove[ab]] = replacements[ab]
            
            
            
            # for ee in theo_to_remove:
            #     latest_index_key = realistic_theoretical_storage[-1]
            #     latest_index_key[ee] = 11
            #     new_index = latest_index_key
            #     realistic_theoretical_storage.append(new_index)
            
            # final_modded_array = realistic_theoretical_storage[-1]
            ctrl_modded_array_store.append(new_array)
            
            
            
            correlation_ctrl_array = signal.correlate(theoretical_array,new_array,method='direct',mode='same')
            correlation_ctrl_norm = correlation_ctrl_array.max()
            correlation_ctrl_max = correlation_ctrl_norm
            correlation_ctrl_mean = correlation_ctrl_array.mean()
            correlation_ctrl = correlation_ctrl_max-correlation_ctrl_mean
            correlation_ctrl_log =  np.log10(correlation_ctrl)
    
            ctrl_correlation_array_store.append(correlation_ctrl_array)
            correlation_ctrl_max_store.append(correlation_ctrl_max)
            correlation_control_mean_store.append(correlation_ctrl_mean)
            ctrl_correlation_subtract_store.append(correlation_ctrl)
            ctrl_correlation_log_store.append(correlation_ctrl_log)
            now = datetime.now()
            current_time =   now.strftime("%H:%M:%S")    
            print('XCorr loop', current_time)
    ...
    # corr_report['Original theoretical array'] = ctrl_og_array_store
    # corr_report['Modified control array'] = ctrl_modded_array_store
    # corr_report['Control cross correlation results array'] = ctrl_correlation_array_store
    # corr_report['Ctrl max correlation score'] = correlation_ctrl_max_store
    # corr_report['Ctrl mean correlation score'] = correlation_control_mean_store
    # corr_report['Ctrl difference correlation score'] = ctrl_correlation_subtract_store
    corr_report['log(Ctrl XCorr)'] = ctrl_correlation_log_store
    corr_report['Exp v Ctrl Difference'] = corr_report['log(Ctrl XCorr)'] - corr_report['Log Correlation']
    
    mean_norm_corr = corr_report['Exp v Ctrl Difference'].mean()
    std_norm_corr = corr_report['Exp v Ctrl Difference'].std()
    
    corr_report['Calibrated Corr'] = (mean_norm_corr-corr_report['Exp v Ctrl Difference'])/std_norm_corr
