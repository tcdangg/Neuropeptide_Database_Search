...
##User input##
output_parent_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\XCorr_Opt\XCorr_validation\20230202\Pseudospectra_storage" #folder in which all output directories will be generated
...
base_file_path = r"C:\Users\lawashburn\Documents\DBpep_v2\XCorr_Opt\XCorr_validation\20230125"
...
min_seq_coverage = 25
normalization=50
subsequent_matching_rounds = 1
scan_log_start = 24056
...
raw_converter_path_input = [r"C:\Users\lawashburn\Documents\DBpep_v2\XCorr_Opt\XCorr_validation\20230125\pseudospectra_nomods_1DF_formatted.txt"]
...
def precursor_amm(db,exp_precursor):
    ...
    merge_match = pd.merge_asof(exp_precursor_sorted,db_sorted, left_on='Precursor actual monoisotopic mass', 
...
def xcorr_calc(ion_report,exp_precursor,scan_to_report):        
...
sequences = fasta_w_mass['Sequence'].values.tolist()
masses = fasta_w_mass['Precursor theoretical monoisotopic mass'].values.tolist()

pseduospectra = pd.DataFrame()



for indie in range(0,len(sequences)):
    peptide = sequences[indie]
    prec_mono = masses[indie]
    ion_report = theoretical_spectra_generator(peptide)    
    prec_mz = (prec_mono+(h_mass*2))/2   
    ion_report_formatted = pd.DataFrame()
    ion_report_formatted['ion'] = ion_report['ion']
    ion_report_formatted['Fragment theoretical monoisotopic mass'] = ion_report['Fragment theoretical monoisotopic mass']
    ion_report_formatted['m/z'] = (ion_report_formatted['Fragment theoretical monoisotopic mass']+(h_mass*1))/1
    ion_report_formatted['resolution'] = 10000
    ion_report_formatted['charge'] = fragment_charges[0]
    ion_report_formatted['intensity'] = 50000
    ion_report_formatted['MS2'] = prec_mz
    ion_report_formatted['scan_number'] = indie + scan_log_start
    ion_report_formatted['precursor_charge'] = precursor_charges[0]
    ion_report_formatted['sequence'] = peptide
    
    pseduospectra = pd.concat([pseduospectra,ion_report_formatted])
    
writePath = output_parent_directory + '\\pseudospectra_key_precursor_z_'+ str(precursor_charges[0])+ '_fragment_z_'+str(fragment_charges[0])+'.txt'
pseduospectra.to_csv(writePath, header=['ion','Fragment theoretical monoisotopic mass','m/z','resolution','charge','intensity','MS2','scan_number','precursor_charge','sequence'], index=None, sep=',')
    
pseduospectra_drop = pseduospectra.drop(columns=['ion','Fragment theoretical monoisotopic mass','sequence'])        
writePath2 = output_parent_directory + '\\pseudospectra_precursor_z_'+ str(precursor_charges[0])+ '_fragment_z_'+str(fragment_charges[0])+'.txt'
pseduospectra_drop.to_csv(writePath2, header=['m/z','resolution','charge','intensity','MS2','scan_number','precursor_charge'], index=None, sep=',')