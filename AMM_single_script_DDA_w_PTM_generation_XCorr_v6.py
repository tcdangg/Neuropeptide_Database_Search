...
##User input##
output_parent_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\Troubleshoot\v5vsv6\v6" #folder in which all output directories will be generated
...
for raw_converter_path in raw_converter_path_input:
    ...
    for peptide in candidate_sequences:
        ...
        for y in scans_present:
            ...
            filtered_experimental = pd.merge_asof(experimental_spectra,banned_mz,left_on='experimental m/z',
                                                    right_on='Banned m/z', tolerance=0.02,allow_exact_matches=True, direction='nearest')
            
            experimental_spectra = filtered_experimental
            experimental_spectra = experimental_spectra.replace(np.nan, 0)
            experimental_spectra = experimental_spectra[experimental_spectra['Banned m/z'] == 0]
